// health.js - External JavaScript for the Paws and Claws pet health website

// Global cart array to store added items
let cart = [];
let wishlist = [];
let currentPage = 'all-health'; // Default page
...
function initializePage() {
    ...
    document.getElementById('all-health').style.display = 'block';
...
function switchCategory(category) {
    ...
    if (category === 'all') {
        document.getElementById('all-health').style.display = 'block';
        currentPage = 'all-health';
    } else if (category === 'dog-health') {
        document.getElementById('dog-health').style.display = 'block';
        currentPage = 'dog-health';
    } else if (category === 'cat-health') {
        document.getElementById('cat-health').style.display = 'block';
        currentPage = 'cat-health';
    } else if (category === 'supplements') {
        document.getElementById('supplements').style.display = 'block';
...
// Validate prescription upload (specific to health products)
function validatePrescription(fileInput) {
    ...
    if (!validTypes.includes(file.type)) {
        showToast('Please upload a valid prescription file (JPG, PNG, or PDF)');
        return false;
    }
    ...
    if (file.size > maxSize) {
        showToast('Prescription file must be less than 5MB');
        return false;
    }
...
function processCheckout() {
    ...
    const requiresPrescription = cart.some(item => {
        // Check if the item title contains keywords that might suggest prescription
        const keywords = ['prescription', 'rx', 'medicine', 'antibiotic', 'treatment'];
        return keywords.some(keyword => item.title.toLowerCase().includes(keyword));
    });
    
    if (requiresPrescription) {
        const prescriptionFile = document.getElementById('prescription');
...
// Filter products by health concern
function filterByHealthConcern(concern) {
    const productCards = document.querySelectorAll(`#${currentPage} .product-card`);
    
    productCards.forEach(card => {
        const tags = card.getAttribute('data-tags');
        if (tags && tags.includes(concern)) {
            card.style.display = 'block';
        } else {
            card.style.display = 'none';
        }
    ...
    // Show reset button
    const resetButton = document.querySelector(`#${currentPage} .reset-filter`);
    if (resetButton) {
